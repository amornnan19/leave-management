...
{
    /**
     * Validate business rules for a new leave request.
     *
    ...
    public function validate(array $data): array
    {
        ...
        if ($userId && $startDate && $endDate) {
            $overlapError = $this->checkOverlap($userId, $startDate, $endDate);
        ...
        if ($leaveType && $startDate && $totalDays !== null) {
            if (! isset($errors['leave_type_id'])) {
                $balanceError = $this->checkBalance($userId, $leaveType, $startDate, $totalDays);
     ...
     */
    private function checkOverlap(int|string $userId, Carbon $startDate, Carbon $endDate): ?string
    {
     ...
     * are counted. The remaining_days accessor on LeaveBalance reflects this.
     */
